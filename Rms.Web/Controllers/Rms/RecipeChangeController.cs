...
{
    public class RecipeChangeController : BaseController
    {
        ...
        public JsonResult GetRecipeChangeRecords(int page, int limit,string EQID)
        {
            var db = DbFactory.GetSqlSugarClient();
            var totalnum = 0;
           
            var data = db.Queryable<RMS_CHANGE_RECORD>().Where(it=>it.EQID == EQID).ToList();
            totalnum = data.Count();
