...
{
    internal class GeneralNonSecs : IRecipeType
    {
        ...
        public GeneralNonSecs(ISqlSugarClient _db, RabbitMqService _rabbitMq)
        {
            db = _db;
            rabbitMq = _rabbitMq;
        }
        ...
        public (bool result, string message) DownloadRecipeToMachine(string EquipmentId, string RecipeVersionId)
        {
            try
            {
                ...
                var rabbitmqroute = $"EAP.Client.{EquipmentId}";
                ...
                if (rabbitRes != null)
                {
                    ...
                    string message = result? "已覆盖设备Recip！": "覆盖失败，请重试！";
